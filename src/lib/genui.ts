...
/**
 * Strips `<genui ...>...</genui>` blocks from a message body so a transcript
 * sent to a classifier carries the prose the learner and tutor actually
 * exchanged, not the serialized interactive widgets.
 */
export function stripGenUITags(content: string): string {
  return content.replace(/<genui\s+type="[^"]+">[\s\S]*?<\/genui>/g, '').trim();
}
